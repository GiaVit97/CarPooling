    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        vm.getTrip().observe(viewLifecycleOwner) { trip ->
            ...
            if (trip != null) {
                ...
                setMenuVisibility(trip.userID == Firebase.auth.currentUser?.uid)
                ...
                if (args.bookingArrival!=null && args.bookingDeparture != null ) {
                    ...
                    departure = GeoPoint(args.bookingDeparture?.lat!!,
                        args.bookingDeparture?.long!!
                    )
                    arrival = GeoPoint(args.bookingArrival?.lat!!,
                        args.bookingArrival?.long!!
                    )

            } else {

                if (othervm.getTripFilter().value?.departureLocation == null || othervm.getTripFilter().value?.arrivalLocation == null) {
                    departure = trip.stops[0].location
                    arrival = trip.stops[trip.stops.size - 1].location
                } else {
                    departure = trip.stops[stopsNext(
                        trip.stops,
                        othervm.getTripFilter().value?.departureLocation!!
                    )].location
                    arrival = trip.stops[stopsNext(
                        trip.stops,
                        othervm.getTripFilter().value?.arrivalLocation!!
                    )].location
                }
            }
            ...
            val availableSeats = minAvailableSeats(trip, true, othervm.getTripFilter().value)
            ...
            if (availableSeats == 0) {
                binding.fab.visibility = View.GONE
            } /*else if (args.bookingArrival != null && args.bookingArrival != null && args.bookingStatus != null && args.bookingStatus == Status.Accepted.status) {
                snackBar(view, "Accepted")
            } else if (args.bookingArrival != null && args.bookingArrival != null && args.bookingStatus != null && args.bookingStatus == Status.Pending.status) {
                snackBar(view, "Pending")
            } */else {
                //manage fab Button
